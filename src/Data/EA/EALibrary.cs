...
{
    internal class EALibrary : IGameLibrary
    {
        public GameLibrary GameLibrary => GameLibrary.EA;
        public string Name => "EA";

        List<Game> _loadedGames = new();
        public List<Game> LoadedGames { get { return _loadedGames; } }

        List<Game> _loadedDLSSGames = new();
        ...
        public bool IsInstalled()
        {
            ...
            var packages = packageManager.FindPackagesForUser(WindowsIdentity.GetCurrent().User.Value,
                "Microsoft.EADesktop_8wekyb3d8bbwe");
            return packages.Any();
        }
        ...
        public async Task<List<Game>> ListGamesAsync()
        {
            ...
            foreach (var drive in drives)
            {
                ...
                if (drive.DriveType == DriveType.Unknown || drive.DriveType == DriveType.Network ||
                ...
                if (File.Exists(gamingRootFile))
                {
                    ...
                    //RGBX
                    if (fileBytes.Length > 5 && fileBytes[0] == 'R' && fileBytes[1] == 'G' && fileBytes[2] == 'B' &&
                        fileBytes[3] == 'X')
                    {
            ...
            foreach (var package in packages)
            {
                ...
                if (gameNamesToFindPackages.ContainsKey(packageName))
                {
                    ...
                    try
                    {
                        var game = new EAGame(gameNamesToFindPackages[packageName])
                        {
                            Title = package.DisplayName, InstallPath = package.InstalledPath,
                        };
                        game.DetectDLSS();
                        games.Add(game);
                    }
